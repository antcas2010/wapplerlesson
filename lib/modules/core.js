...
module.exports = {
    ...
    exec: async function(options) {
        ...
        if (options.exec && fs.existsSync(`app/modules/lib/${options.exec}.json`)) {
            return await this.sub(await fs.readJSON(`app/modules/lib/${options.exec}.json`), {
                $_PARAM: this.parse(options.params)
            });
        } else {
    ...
    group: async function(options, name) {
        return await this.sub(options.exec);
    },

    parallel: async function(options, name) {
        let actions = options.exec.steps || options.exec;
        if (!Array.isArray(actions)) actions = [actions];
        return await Promise.all(actions.map(exec => this.sub(exec)));
    },
